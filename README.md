...
为了减少 LLM 调用链路和 WebSocket 开销，建议在系统提示词或工具使用说明里明确写死以下规则：

1. **优先直接控制，不要先查再控。**
   - 只要用户意图明确，就直接调用对应控制工具。
   - 不要为了“确认一下”先调用设备状态查询工具。

2. **默认不要使用设备解析工具。**
   - 已移除 `resolve_device` 后，LLM 应直接使用 `list_devices` 结合上下文判断目标设备。
   - 如果用户给出的实体名已经足够明确，直接控制即可。

3. **关灯/开灯时，先把 `light` 和 `switch` 都当成灯类设备看待。**
   - `list_devices` 会同时返回 `light` 与 `switch`。
   - 对于很多实际是 `switch.*` 的灯，控制时仍然要允许走 `turn_on` / `turn_off`。

4. **批量控制所有灯时，必须优先使用 `set_all_lights_state`。**
   - 当用户说“打开所有灯”“关闭所有灯”“全屋开灯”“全屋关灯”“把房间内灯全开/全关”时，必须直接调用 `set_all_lights_state`。
   - 不允许先调用 `list_devices` 后逐个调用单灯控制工具。
   - 不允许用 `control_device` 逐个替代。
   - 如果用户指定了房间，要把房间名原样传给 `room`。

5. **避免重复刷新设备列表。**
   - 只有当设备列表明显过期或上下文变化很大时才刷新。
   - 同一轮对话中，不要频繁重复触发设备发现。

6. **控制结果以“请求已发送/已执行”为主，不要强制轮询确认。**
   - 除非用户明确要求确认状态，否则不要为了确认而反复查询。
   - 这样可以显著减少等待和超时。

## 批量灯控示例

### 用户说：打开所有灯
必须调用：`set_all_lights_state({ state: "on" })`

### 用户说：关闭客厅所有灯
必须调用：`set_all_lights_state({ state: "off", room: "客厅" })`

### 用户说：把全屋灯关掉
必须调用：`set_all_lights_state({ state: "off" })`

以上规则适合写进系统提示词，或作为 MCP 工具使用说明直接固定下来，避免模型在运行时频繁走冗余查询链路。
